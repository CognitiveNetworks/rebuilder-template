...
- **Project structure:** Layered monolith under `src/app/`. Routes handle HTTP, event_types classify and validate, output generates JSON, obfuscation handles blacklisting, security handles HMAC, delivery sends to Kafka. External modules `kafka_module` and `rds_module` are local path dependencies.
- **Data flow:** SmartCast TVs → POST `/` → validate request (params, HMAC hash, timestamps) → classify event type → generate flattened output JSON → apply channel obfuscation → send to Kafka topics via `kafka_module.producer.send_message()`.
- **Service boundaries:** Single FastAPI service. RDS access via `rds_module.client.execute_query()`. Kafka delivery via `kafka_module.producer.send_message()`. No inter-service communication.
- **Infrastructure:** AWS EKS (Kubernetes), Helm charts, Terraform for IaC, KEDA for autoscaling. Docker container with `python:3.12-slim` base.
- **API design:** POST `/` (backward-compatible TV ingestion), GET `/status` (simple health), GET `/health` (deep health), GET/POST `/ops/*` (SRE contract). OpenAPI auto-generated by FastAPI at `/docs`.
...
- **Local services:** `docker compose up -d` (starts PostgreSQL, Kafka, app)
- **Database setup:** Automatic via `scripts/init-db.sql` (mounted in docker-compose)
- **Seed data:** Included in `scripts/init-db.sql` (blacklist channel IDs)
- **Test:** `pytest`
- **Lint:** `pylint src/app tests --disable=import-error --fail-under=10.0`
- **Format:** `black --check src tests --skip-string-normalization`
- **Type check:** `mypy src/app/ --ignore-missing-imports`
- **Complexity:** `complexipy src -mx 15`
- **Run:** `uvicorn src.app.main:app --reload --host 0.0.0.0 --port 8000`
...
- **Logging:** Structured JSON to stdout via Python `logging` with OTEL trace/span correlation
- **Metrics and tracing:** OpenTelemetry SDK with OTLP HTTP exporter to New Relic. Auto-instrumentation via `opentelemetry-instrument`. Custom counters for business operations (ingestion, validation, delivery, obfuscation, cache hits/misses).

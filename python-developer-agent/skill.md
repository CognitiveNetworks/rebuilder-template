...
- **All static analysis, linting, and formatting tools must run against both `src/` and `tests/` directories.** Never limit tooling to only application code. Tests must meet the same quality standards as production code. Configure tools in `pyproject.toml` to include both directories: `pylint src tests`, `black src tests`, `mypy src tests`, etc.

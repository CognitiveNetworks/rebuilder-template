...
- **Container path structure must match production expectations.** The application code must be accessible at `/app` inside the container, not `/src/app`. The `Dockerfile` must copy `src/app/` to `/app/` (e.g., `COPY ./src/app ./app`) so the Python path works correctly in both local development and CI/CD.
- **Python imports must use `app.*`, never `src.app.*`.** All source code imports must use `from app.module import ...`, not `from src.app.module import ...`. The `src/` directory is a local development layout convention — it must not leak into import paths. In the container, the app lives at `/app/` and `app.*` resolves directly. For local development, configure `pytest.ini` with `pythonpath = src` and `.pylintrc` init-hook with `sys.path.insert(0, "src")` so that `app.*` imports resolve from the `src/` directory. The `entrypoint.sh` must use `uvicorn app.main:app`, not `uvicorn src.app.main:app`. This ensures imports work identically in local dev, CI, and the container.

...
- **Container path structure must match production expectations.** The application code must be accessible at `/app` inside the container, not `src/app`. The `Dockerfile` must copy `src/app/` to `/app/` so the Python path works correctly in both local development and CI/CD. This ensures `PYTHONPATH=/app` works consistently and imports resolve the same way across environments.
...
- **Dependency locking script must be executable and run after any dependency change.** The `scripts/lock.sh` script must be executable (`chmod +x scripts/lock.sh`) and must be run after any modification to `pyproject.toml` or dependency changes. Verify it ran successfully by checking that `requirements.txt` and `requirements-dev.txt` were updated with new timestamps and hashes. Run the script twice to ensure idempotent behavior - the second run should produce identical outputs or no changes if dependencies are already locked.
- **Pylint must be configured for CI environments.** Configure pylint in `pyproject.toml` with `--disable=import-error` for local modules that aren't installable in CI (e.g., kafka, rds modules). Use `--fail-under=10.0` to require a perfect score with no errors, warnings, or conventions issues. Ensure all production dependencies are installed before running pylint in CI - install both production and dev requirements: `pip install -r requirements.txt -r requirements-dev.txt`. The PYTHONPATH must include the source directory: `export PYTHONPATH=$PWD/src:$PYTHONPATH`.
...
- **All static analysis, linting, and formatting tools must run against both `src/app/` and `tests/` directories.** Never limit tooling to only application code. Tests must meet the same quality standards as production code. Configure tools in `pyproject.toml` to include both directories: `pylint src/app tests`, `black src/app tests`, `mypy src/app tests`, etc. Note: In the container, the code is copied to `/app`, but static analysis tools always run against the source structure `src/app/`.

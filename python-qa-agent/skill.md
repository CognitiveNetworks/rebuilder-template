...
- **Workspace isolation.** During verification, read only from the built repo being verified, its `template/skill.md`, and the legacy repo (`repo/`) for comparison. Never read from, reference, or compare against any other `rebuilder-*` repository in the workspace. Other rebuilder repos may be partially built, stale, or from a different project. Each rebuild is verified in isolation.
...
**CI pipeline immutability rule:** The QA agent verifies the CI pipeline — it never modifies it. Do not add, remove, rename, or change any job in `.github/workflows/ci.yml` or any other workflow file. The CI jobs listed in gates 18–23 above are examples; the actual jobs to verify come from the repo's existing `ci.yml`. If a CI job fails, the fix is in the application code, test code, or tool configuration (e.g., `pyproject.toml`, `.pylintrc`) — never in the workflow YAML itself. If the repo's CI has fewer or different jobs than listed above, verify exactly the jobs that exist. If the repo's CI has jobs not listed above, verify those too. The CI pipeline is owned by the repo — the QA agent reads it, runs it, reports on it, but does not change it.

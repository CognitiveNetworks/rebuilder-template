...
├── README.md                  # This file
├── spec-driven-development.md # Leadership doc — reproducibility, spec-lock, agent architecture
├── SPEC_DEBT.md               # Spec-debt tracker — divergences between specs and code
├── spec-locks.yaml            # Spec-lock manifest — auto-generated by scan-spec-locks.sh
...
│   ├── PULL_REQUEST_TEMPLATE.md   # PR template — spec-impact checklist and engineer sign-off
│   └── workflows/
│       └── spec-impact.yml        # CI gate — blocks PRs that touch spec-locked code without justification
├── actions/
│   └── spec-impact/
│       └── action.yml         # Reusable action — spec-impact analysis for PRs
├── scripts/
│   └── scan-spec-locks.sh     # Extract @spec-lock annotations from codebase into spec-locks.yaml
├── rebuild/

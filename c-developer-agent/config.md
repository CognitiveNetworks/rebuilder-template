# C Developer Agent — Project Configuration
...
> `c-developer-agent/skill.md` file defines the universal standards; this
...
| **Unit test** | `cd build && ctest --output-on-failure` |
| **Coverage** | `lcov --capture --directory build/ --output-file coverage.info && genhtml coverage.info --output-directory coverage_report` |
| **Format check** | `clang-format --dry-run -Werror src/**/*.c src/**/*.h include/**/*.h` |
| **Static analysis** | `cppcheck --enable=all --error-exitcode=1 src/` |
| **Lint** | `clang-tidy src/**/*.c -- -Iinclude/` |
| **Complexity** | `lizard src/ -C 15 -w` |
...
The authoritative checklist is in `c-developer-agent/skill.md` → Required
...
|------|-----------|------|-------------|
| *[TODO]* | `src/` | 8000 | *[TODO: service description]* |

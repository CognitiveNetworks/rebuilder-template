...
This step applies **only when the input codebase was previously generated by this process** and has accumulated human edits since the last generation. If this is the first generation, skip to Step 1.

**When to run:** A `SPEC_DEBT.md` file exists in the input codebase with entries in the "Active Debt" section.

**What to do:**

1. Read `SPEC_DEBT.md` from the input codebase. Each entry documents a divergence between the PRD and the code — a place where a human engineer changed the generated code and explained why.
2. For each active entry:
   - Read the referenced PRD section.
   - Read the engineer's "why" explanation and the before/after description.
   - Determine whether the PRD should be updated to match reality, or whether the code change was a local workaround that a better spec would prevent.
3. Update the PRD (`output/prd.md` or the input PRD if provided) to incorporate the lessons from accumulated debt entries. This means:
   - Adding missing requirements that engineers had to implement by hand.
   - Correcting requirements that produced wrong behavior.
   - Adding constraints that the spec omitted (race conditions, edge cases, ordering guarantees).
4. After merging, mark each reconciled entry as resolved in `SPEC_DEBT.md` with a resolution note.
5. Also read `spec-locks.yaml` (if it exists from a previous `scan-spec-locks.sh` run). For each lock whose `reason` has been addressed by the PRD update, note it — that lock can be removed after the rebuild confirms the fix.

**Output:** An updated PRD with accumulated human knowledge merged in, and a reconciled `SPEC_DEBT.md` with resolved entries moved to the "Resolved" section.

**Why this matters:** Without reconciliation, re-replication would overwrite every human fix with the same wrong generated code. This step closes the loop: human edits become spec improvements, and the next generation is better than the last.
...
**Spec-Lock Inventory (only if input is a previously replicated codebase)**

If the legacy codebase was previously generated by this process (or any spec-driven generation), scan for `@spec-lock` / `@end-spec-lock` annotation pairs. These mark code blocks that a human engineer deliberately preserved because the generated implementation was wrong, incomplete, or inexpressible in PRD prose.

For each locked block, extract:
- The `spec_section` it references (which PRD section governs this code)
- The `lock_id` (unique identifier)
- The `contract` (what the code guarantees)
- The `reason` (why the generated version was replaced)

Run `scripts/scan-spec-locks.sh` on the legacy codebase to produce `spec-locks.yaml`. Include the scan output in the assessment. These locks represent **human-authoritative implementations** — they must be ported faithfully during the rebuild, not regenerated from PRD prose.

If no `@spec-lock` annotations are found, omit this section from the output.
...
## Spec-Lock Inventory
[Include this section only if `@spec-lock` annotations were found. Omit entirely for first-generation rebuilds or codebases with no locks.]

- **Total locks found:** [count]
- **Scanner output:** `spec-locks.yaml` generated by `scripts/scan-spec-locks.sh`

| Lock ID | Spec Section | Contract | Reason | File:Line |
|---|---|---|---|---|
| [lock_id] | [spec_section] | [contract summary] | [why generated code was wrong] | [file:line] |

**Implications for rebuild:**
- These locked blocks must be ported verbatim (or functionally equivalent) into the rebuilt codebase.
- The PRD sections referenced by these locks may need updating — the lock's `reason` field explains what the spec got wrong.
- After the rebuild, locks whose underlying PRD section has been corrected can be removed.

## Summary
- Overall Risk Level: [Low / Medium / High / Critical]
- Top 3 Risks: [list]
- Strongest Assets to Preserve: [list — what's working well and should carry forward]
- Spec-Lock Count: [number, or "N/A — first generation"]
```
...
If Step 1 produced a spec-lock inventory (`spec-locks.yaml`), verify that every locked block was faithfully ported into the rebuilt codebase:

- [ ] Each lock ID from `spec-locks.yaml` has a corresponding implementation in the new code
- [ ] The contract described in each lock is satisfied by the new implementation
- [ ] Locks whose underlying PRD section was corrected in Step 0 reconciliation have been re-evaluated — if the PRD now captures the requirement correctly, the lock annotation is removed and the code is generated from spec
- [ ] Remaining locks are carried forward with their `@spec-lock` annotations intact in the new codebase

Do not silently drop locked implementations. If a locked block cannot be ported (e.g., the target language or framework has no equivalent), document the gap in `SPEC_DEBT.md` with a resolution plan.

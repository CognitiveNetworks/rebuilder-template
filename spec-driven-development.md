...
That contract solves Day 1. But the process also improves over time: every
manual correction an operator gives during a rebuild is analyzed and codified
back into the process or standards documents (Step 17), so each successive
generation is better than the last.
...
| Quality gates | skill.md Step 12 | pytest, pylint, black, mypy, complexipy, pip-audit, etc. |

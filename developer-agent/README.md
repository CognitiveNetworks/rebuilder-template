...
Daily development instructions for AI-assisted coding sessions. `skill.md` is loaded automatically via IDE-specific instruction files at the project root.
...
This is not the migration planning document (`STANDARDS.md`) and not the SRE agent (`sre-agent/skill.md`). The developer agent handles the daily work of building services that conform to the project's standards.
...
If you ran `rebuild/run.sh`, Steps 7a, 7b, and 7c of the rebuild process have already populated the project-specific sections of `skill.md`, `config.md`, and generated the IDE instruction files.

1. Review the auto-populated content in `skill.md` and `config.md` for accuracy.
